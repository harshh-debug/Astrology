...
form.addEventListener('submit',(event)=>{
    ...
    const zodiacMessage=`Your Zodiac sign is ${zodiacSigns[month-1]}`
    const compliment_message=`${compliments[day-1]}`
    const index=Math.floor(Math.random()*(victimCardCompliments.length))
    const victim_message=`${victimCardCompliments[index]}`
    const rindex= ((name.length)*(Surname.length)*day)%30
    const recommendation_message=`${recommendations[rindex]}`
    const pindex=(year*month)%20
    const prediction_message=`${predictions[pindex]}`
    // result.innerHTML=`${greeting} ${zodiacMessage} ${compliment_message} ${victim_message} ${recommendation_message} ${prediction_message}`

    result.innerHTML = `
        <div class="result-section">
            <h3>Your Profile</h3>
            <p>${greeting} ${zodiacMessage}</p>
        </div>
        <div class="result-section">
            <h3>Insights about you</h3>
            <p>${compliment_message} ${victim_message}</p>
        </div>
        <div class="result-section">
        ...
        <div class="result-section">
            ...
            <p class="prediction">${prediction_message}</p>
        </div>`
